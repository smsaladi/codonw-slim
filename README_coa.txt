...
cusort.coa�
Contains the codon usage of each gene, sorted by the gene�s coordinate
...
(as defined by the �advanced correspondence analysis options�) of g
...
in the file �hilo.coa�. The format of this file can be utilised by Codon
...
tables� report the fraction of variation in each gene and codon (or ami
...
putative optimal codons. Though the adage GIGO �garbage in, garbage ou
...
written to the file �cusort.coa�. Then a number of genes, decided by
advanced correspondence analysis menu option �number of genes used
identify optimal codons�, are read from the start and end of this file (i.
...
�summary.coa� and �hilo.coa�. those codons which have been pu
...
and codon adaptiveness values are in-built into codonW (see the �Chan
Defaults� menu). For other species these indexes cannot be calculated unle
the additional information is know. During calculation of these indices the
user is prompted for input files.
During a COA CodonW generates the output files �cai.coa�, �fop.co
�cbi.coa�. These files can be used as input files for their respec
