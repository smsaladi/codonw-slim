...
void textbin(char *fileread, char *fileout)
{
    ...
    if (pm->coa == 'a')
    {
        for (i = 1; i < 22; i++)
            if (pcoa->amino[i])
                pcoa->colm++; /* number of colms in analysis */
    }
    else
    {
        for (i = 1; i < 65; i++)
    ...
    { /* pcoa-rows is the No of genes */
        ...
        case 'a':
            ...
            if (x != pcoa->colm)
                my_exit(99, "Fatal Error in txt2bin");
            break;
        case 'c':
            for (j = 1, x = 1; j < 64; j++)
            {
        ...
        case 'r':
            ...
            for (j = 1, x = 0; j <= 64; j++)
            {
                ...
                {
                    ++x;
                    vlec[x] = (double)((naa[pcu->ca[j]]) ? (float)ncod[j] / naa[pcu->ca[j]] * (float)(*(ds + j)) : 0.00);
                }
            }
            break;
        } /* end if */
...
void DiagoComp(int n0, double **w, double *d, int *rang)
{
    ...
    Etd:
        ...
        for (ijk = 1; ijk <= jk; ijk++)
        {
            ...
            xp = u * s[i];
            b = v * s[i];
            if (fabs((double)xp) < fabs((double)q))
                goto Et8;
            u = xp / q;
