...
void menu_1(void)
{
    ...
    for (n = (int)strlen(root); n && root[n] != '.'; --n)
        ;
    if (n)
        root[n] = '\0'; /* define root of the filename */
...
void menu_3(void)
{
    ...
    while (loop)
    {
        ...
        printf("{%s readable}\n", (pm->seq_format == 'M') ? "Computer" : "Human");
        printf(" %-40.40s", "(9) Concatenate or individual genes");
        printf("{%s genes}\n", (pm->totals == TRUE ? "concatenate" : "individual"));
...
void menu_4(void)
{
    ...
    while (loop)
    {
        ...
        for (i = 1; i <= NumChoices; i++)
        {
            ...
            {
            case 1:
                (pm->cai) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 2:
                (pm->fop) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 3:
                (pm->cbi) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 4:
                (pm->enc) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 5:
                (pm->gc) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 6:
                (pm->gc3s) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 7:
                (pm->sil_base) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 8:
                (pm->L_sym) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 9:
                (pm->L_aa) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 10:
                (pm->hyd) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 11:
                (pm->aro) ? printf("{%-45.45s}", choices[i]) : printf(" %s ", choices[i]);
                break;
            case 12:
                printf(" %s ", choices[i]);
                break;
...
void menu_5(void)
{
    ...
    while (loop)
    {
        ...
        for (i = 1; i <= NumChoices; i++)
        {
            ...
            {
            case 1:
                (pm->coa == 'c') ? printf("{%-45.45s}", choices[1]) : printf(" %s ", choices[1]);
                break;
            case 2:
                (pm->coa == 'r') ? printf("{%-45.45s}", choices[2]) : printf(" %s ", choices[2]);
                break;
            case 3:
                ...
                break;
            case 4:
                (pm->coa == 0) ? printf("{%-45.45s}", choices[4]) : printf(" %s ", choices[4]);
                break;
            default:
                fprintf(stderr, "programming error \n");
                my_exit(99, "menu 5");
        ...
        if (pm->coa)
        {
            ...
            if ((char)toupper((int)pm->junk[0]) == 'Y')
                menu_coa();
        }
...
void menu_8(void)
{
    ...
    { /* matches  if( strlen( pm->curr_cufilename)  )               */
        ...
        else
        {
            ...
            case 2: /* Num of axis to record        */
                printf("Changing the number of axis generated from %i "
                       "Please input new value [%i]",
                       (int)pcoa->axis, (int)pcoa->axis);
                gets(pm->junk);
                if (!strlen(pm->junk))
                    break;
                if (isalpha((int)pm->junk[0]))
            ...
            case 5: /* how to identify optimal codons */
                ...
                if ((p = strchr(pm->junk, '%')) != NULL)
                {
                    *p = '\0';
                    ...
                    if (pcoa->fop_gene == 0 || pcoa->fop_gene < 50)
                    { /* err_catch */
...
void select_coa(char choice)
{
    ...
    while (loop)
    {
        ...
        while (toggle = (int)strtol(startpoint, &endpoint, 10))
        {
            ...
            if (pm->coa == 'a')
            {
                if (toggle > 21 || toggle < 1)
                    continue; /* check value is valid */
                pcoa->amino[toggle] = (char)((pcoa->amino[toggle]) ? FALSE : TRUE);
            }
            else
            {
                if (toggle > 64 || toggle < 1)
                    continue; /* check value is valid */
